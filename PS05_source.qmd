...
ggplot(hate_crimes, aes(x = trump_support, y=hate_crimes)) +
  geom_boxplot()+
  labs(x = "Voter Support for Trump", y= "Number of Hate Crimes (per 100k people)", title = "Problem 1")
```
:::
...
-   It's not surprising, cities tend to be areas of low Trump support and have higher populations which would mean more chances for a hate crime to occur.
:::
...
-   The number of hate crimes per 100k people in areas of high trump support.
:::
...
-   The model estimates 0.46 hate crimes per 100k people in areas of low Trump support.

```{r}
# Type your code and comments inside the code chunk
hate_mod_tab <- get_regression_table(hate_mod)
hate_mod_tab
low <- hate_mod_tab[1,2] + hate_mod_tab[2,2]
```
:::
...
-   The intercept is the predicted number of hate crimes per 100k (at 0.191) in areas of high Trump support, the model estimates that areas of low Trump support have higher hate crime rates at 0.46 hate crimes per 100k.
:::
...
```{r, label = "delta"}
# Type your code and comments inside the code chunk
medium <- hate_mod_tab[1,2]+hate_mod_tab[3,2]
high <- hate_mod_tab[1,2]
```

-   Hate crimes were 0.031 higher per 100k people in areas of medium Trump support.
:::
...
-   For areas of low Trump support: 0.460.

-   For areas of medium Trump support: 0.222.

-   For areas of high Trump support: 0.191.
:::
...
-   The values are very similar, calculating by group means just results in a higher number of decimal places/more sig figs.
:::
...
$$
\widehat{y} = `r coef(hate_mod)[1]` + `r coef(hate_mod)[2]` \times D_1 + `r coef(hate_mod)[3]` \times D_2 \\
...
-   When Trump support is high, the rate of hate crimes is 0.191 per 100k people.
:::
...
```{r}
# Type your code and comments inside the code chunk
p11 <- arrange(hate_crimes, desc(hate_crimes))
p11
```

-   Oregon, Washington, Massachusetts, Minnesota, and Maine had the highest rate of hate crimes per 100k people, these results are a little surprising but they make sense in context with other assumptions that have been drawn.
:::
...
-   The intercept is the rate of hate crimes per 100k people in areas of high unemployment.
:::
...
-   The estimate for the number of hate crimes per 100k people in an area of low unemployment is 0.32.
:::
...
-   The two possible values are 0.320 and 0.289, there are only two values because there are only two options for the categorical variable unemployment to be (high and low).
:::
...
-   The residual is found by subtracting the estimated hate crime value from the actual hate crime data value.
:::
...
-   In areas of high median household income there was a 0.16041 higher amount of hate crimes per 100k people.
:::
...
-   The model underpredicted, the actual number of hate crimes per 100k in Maine is 0.616, the model predicted it at 0.229.
:::
